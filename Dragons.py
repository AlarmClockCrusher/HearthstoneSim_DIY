	...
	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		...
		if self.Game.Hand_Deck.hands[3-self.ID]:
			if self.Game.turn == self.ID:
				#假设被沙德沃克调用时无效果
				if "InvokedbyOthers" not in comment:
					cardinHand = np.random.choice(self.Game.Hand_Deck.hands[3-self.ID])
					if len(self.Game.Hand_Deck.decks[3-self.ID]) < 2:
						cardsinDeck = np.random.choice(self.Game.Hand_Deck.initialDecks[3-self.ID], 2, replace=False)
						cardsinDeck = [card(self.Game, self.ID) for card in cardsinDeck]
					else: cardsinDeck = np.random.choice(self.Game.Hand_Deck.decks[3-self.ID], 2, replace=False)
					self.Game.options = list(cardsinDeck) + [cardinHand]
					np.random.shuffle(self.Game.options)
					self.Game.DiscoverHandler.startDiscover(self)
		else: PRINT(self, "The opponent has no card, and Envoy of Lazul's battlecry has no effect")
		return None
		
	def discoverDecided(self, option):
		if option.inHand:
			PRINT(self, "Bingo. Copy of %s from enemy hand is put into player's hand."%option.name)
			...
			for card in self.Game.options:
				if card.inHand:
					rightCard = card
					break
			PRINT(self, "Guess is wrong. The card in player's hand is %s"%rightCard.name)
