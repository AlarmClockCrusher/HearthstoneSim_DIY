...
from Game import *
from Basic import ClassDict, ElvenArcher
...
OwnHeroPos, EnemyHeroPos = (0.51*X, Y-0.24*Y), (0.51*X, 0.24*Y)
OwnWeaponPos, EnemyWeaponPos = (0.42*X, Y-0.24*Y), (0.42*X, 0.24*Y)
OwnPowerPos, EnemyPowerPos = (0.6*X, Y-0.24*Y), (0.6*X, 0.24*Y)
...
class HandButton(tk.Button): #Cards that are in hand. 目前而言只有一张牌是自己可以打出的牌的时候点击是有响应的。
	...
	def plot(self, x=11, y=11, anchor='c'):
		...
		if self.card.type == "Minion":
			...
			if health >= health_upper: healthColor = "black" if health_max <= self.card.health_0 else "green"
...
class MulliganFinishButton(tk.Button):
	def respond(self):
		...
		self.GUI.printInfo("Have decided cards to mulligan. Send info to the opponent")
		#应该在这里生成一个起手调度信息
...
class TurnEndButton(tk.Button):
	def respond(self):
		...
		self.GUI.window.clipboard_clear()
		self.GUI.window.clipboard_append(s)
		self.GUI.printInfo("New info for opponent has been generated.")
		for move in moves:
...
class ClassConfirmationButton(tk.Button):
	def respond(self):
		deck, hero = [], ClassDict[self.GUI.hero1Label["text"].split(':')[-1]]
		deckString, decksCorrect = self.GUI.deck1.get(), True
		if deckString != "":
		...
		for obj in deck:
			if obj == None: decksCorrect = False
		if decksCorrect:
			...
			self.GUI.update()
		elif not decksCorrect: self.GUI.printInfo("Deck Incorrect")
...
class EnemyPlaysEntry(tk.Entry):
	def respond(self, event): #读入一个字符串，然后转换为plays
		...
		print("moves and gameGuides receivered are:\n", type(moves), moves, type(gameGuides), gameGuides)
		if isinstance(moves, list):
			...
			if oppo_ID != self.GUI.ID:
				...
				self.GUI.btnID.unbind("<Button-1>")
...
class GUI:
	def __init__(self):
		...
		self.GamePanel.pack(fill=tk.Y, side=tk.LEFT)
		self.outPutPanel = tk.Frame(master=self.window, width=0.01*X, height=0.3*Y, bg="cyan")
		self.outPutPanel.pack(side=tk.TOP)
		self.inputPanel = tk.Frame(master=self.window, width=int(0.01*X), height=int(0.3*Y), bg="cyan")
		self.inputPanel.pack(side=tk.TOP)
		self.deckImportPanel = tk.Frame(master=self.window, width=0.01*X, height=0.6*Y)
		...
		self.output = tk.Listbox(master=self.outPutPanel, xscrollcommand=scrollbar_hor.set, yscrollcommand=scrollbar_ver.set, width=50, height=15, bg="white", font=("Courier", 13))
		...
		self.btnGenInfo = InfoGenButton(master=self.inputPanel, bg='yellow', text="Generate Update", font=("Yahei", 15), width=18, height=1)
		self.btnGenInfo.bind('<Button-1>', self.btnGenInfo.respond)
		...
		lbl_deck1 = tk.Label(self.deckImportPanel, text="Deck code", font=("Yahei", 14))
		self.deck1 = tk.Entry(self.deckImportPanel, font=("Yahei", 12))
		lbl_deck1.pack()
		self.btnID = IDButton(self.deckImportPanel, bg= "green", text="You're Player 1", font=("Yahei", 15))
		self.btnID.GUI = self
		...
		if self.UI == -1: #Draw the mulligan part, the cards and the start turn button
			...
			if self.UI == 1: self.drawChooseOne()
				...
				if self.ID == 1:
					...
					for i in range(len(self.Game.minions[1])):
						self.posMinionsDrawn[1].append((0.043*X+(X/9)*i, 0.40*Y))
					for i in range(len(self.Game.minions[2])):
						self.posMinionsDrawn[2].append((0.043*X+(X/9)*i, 0.58*Y))
					for i in range(len(self.Game.Hand_Deck.hands[1])):
						self.posHands[1].append((0.043*X+(X/13)*i, 0.11*Y))
					for i in range(len(self.Game.Hand_Deck.hands[2])):
						self.posHands[2].append((0.043*X+(X/13)*i, Y - 0.07*Y))
				self.drawHands()
				self.drawMinions()
				self.drawHeroesWeaponsPowers()
				self.drawManasHandsDecksSecretsQuests()
			if self.ID == self.Game.turn:
				btnTurnEnd = TurnEndButton(master=self.GamePanel, text="Turn End", bg="green", fg="white", width=12, height=2, font=("Yahei", 13, "bold"))
	...
	def drawMulligan(self):
		...
		for i, card in enumerate(self.Game.mulligans[self.ID]):
			pos = (70+i*2*111, Y-120)
			color = "red" if self.mulliganStatus[i] else "green"
	...
	def drawManasHandsDecksSecretsQuests(self):
		...
		for ID in range(1, 3):
			...
			while i < manastoDraw + 1:
				pos = (0.69*X+(X/50)*i, Y - 0.26*Y) if ID == self.ID else (0.69*X+(X/50)*i, 0.23*Y)
				if usable > unlocked:
			...
			for i in range(1, manaHD.manasOverloaded[ID]+1):
				pos = (0.69*X+(X/50)*i, Y - 0.23*Y) if ID == self.ID else (0.69*X+(X/50)*i, 0.26*Y)
			...
			color = "green" if ID == self.Game.turn else "red"
			...
			if self.ID == ID: lbl_Mana.place(relx=0.68, rely=1-0.26, anchor='c')
			else: lbl_Mana.place(relx=0.68, rely=0.23, anchor='c')
