...
statusDict = {"Immune": 0, "Immune2NextTurn": 0, "ImmuneThisTurn": 0,
				...
				"Ignore Taunt": 0, #Kayn Sunfury makes player's Ignore Taunt
				}
...
class Game:
	...
	def initialize(self, cardPool, MinionsofCost, RNGPools, hero1=None, hero2=None, deck1=[], deck2=[]):
		...
		self.Manas = Manas(self)
		self.Secrets = Secrets(self)
		self.DmgHandler = DmgHandler(self)
	...
	def adjacentMinions2(self, target, permanentCountsasMinion=False):
		targets, ID, pos = [], target.ID, target.position
		leftMinionExists, rightMinionExists = False, False
		while pos > 0:
			pos -= 1
			obj_onLeft = self.minions[ID][pos]
			if permanentCountsasMinion == False and obj_onLeft.type != "Minion":
		...
		while pos < boardSize - 1:
			...
			if permanentCountsasMinion == False and obj_onRight.type != "Minion":
	...
	def playMinion(self, minion, target, position, choice=0, comment=""):
		ID = minion.ID
		canPlayMinion = False
		if self.Manas.affordable(minion) == False:
	...
	def summon(self, subject, position, initiatorID, comment="SummoningCanbeDoubled"):
	...
	def sendSignal(self, signal, ID, subject, target, number, comment, choice=0, triggerPool=None):
		if triggerPool != None: #主要用于打出xx牌和随从死亡时/后扳机，它们有预检测机制。
			...
			#场上扳机先触发
				...
				if signal == "MinionDies" and self.Secrets.isSecretDeployedAlready(Redemption, 3-self.turn):
	...
	def playSpell(self, spell, target, choice=0, comment=""):
		...
		if canPlaySpell:
			...
			if self.Secrets.isSecretDeployedAlready(Counterspell, 3-self.turn):
	...
	def playHero(self, heroCard, choice=0):
		ID = heroCard.ID
		if self.Manas.affordable(heroCard) == False:
